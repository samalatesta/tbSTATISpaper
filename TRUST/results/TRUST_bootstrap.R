...
for(i in 1:z){
  ...
  trust <- fit_STATIS(data=boot, p_vec, D, nstart, initial_iter)
  ml <- t(data.frame(trust[[4]]) %>% dplyr::arrange(pos) %>% select(sub))
  boot_ml = rbind(boot_ml, ml)
}

write.csv(boot_ml, paste0("/usr3/graduate/samalate/projectnb/cbs/samalate/tbSTATISpaper/TRUST/results/TRUST_boot_ml_",iTask,".csv"), row.names = F)
