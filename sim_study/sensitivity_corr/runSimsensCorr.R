...
setwd("/usr3/graduate/samalate/projectnb/cbs/samalate/tbSTATISpaper/sim_study/sensitivity_corr")
...
sim <- function(M, N,p, z, nstart, initial_iter, cor){
  ...
  for(i in 1:z){
    ...
    #make data set
    dat <- make_data(bio_info_long, p, M, cor)
    data<- dat[[1]]
    ...
    p_vec2 = p_vec
    #run TB-STATIS
    ml <- fit_STATIS(data[,3:ncol(data)],  p_vec2, bio_info_long, nstart, initial_iter )
    ...
    m <- get_likelihood(data[3:ncol(data)], data.frame(ml[[4]]), p_vec2)[[4]]
    
    #get predicted disease class 
    stage=get_stage(data[,3:ncol(data)],S=data.frame(ml[[4]]),p=p_vec2)
...
#run sim 
sim1 <- sim(M, N,p, z, nstart, initial_iter, cor)

#save results 
saveRDS(sim1[[1]], paste0("./sim_results/sim_seq_sens_corr",cor, "_", M,"_", p, "_", N,"_", iTask, ".rds"))
saveRDS(sim1[[2]], paste0("./sim_results/sim_data_sens_corr",cor, "_", M,"_", p, "_", N,"_", iTask, ".rds"))
